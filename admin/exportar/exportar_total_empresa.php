...
->setCellValue('I3', 'Empresa')
->setCellValue('J3', 'Usuario')
->setCellValue('K3', 'Organizador')
->setCellValue('L3', 'Fecha Reunión')
;
$conn= new DataBase();
$link = $conn->Conectarse();
$query = "SELECT idtba_regist_asiste, cedula, nom_comple, telefono, correo, dir_reside, localidad, Lug_votaci, empresa, USER, organizador, CONVERT(fec_creaci, DATE) AS fec_creaci FROM gloriadiaz.vs_exp_empresa ";
...
while ($row=mysqli_fetch_assoc($result)):
	...
	->setCellValue('L'.$c, $row['fec_creaci'])
	;
	$c++;
	$i++;
endwhile;
...
header('Content-Disposition: attachment;filename="Asistencia_Empresa_Total.xlsx"');
