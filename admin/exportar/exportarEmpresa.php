...
if ($fecha_inicial != "" || $fecha_inicial != null){
	...
	->setCellValue('I6', 'Empresa')
	->setCellValue('J6', 'Usuario')
	->setCellValue('K6', 'Organizador')
	;
	$conn= new DataBase();
    $link = $conn->Conectarse();
    $query = "SELECT idtba_regist_asiste, cedula, nom_comple, telefono, correo, dir_reside, localidad, Lug_votaci, empresa, USER, organizador, fec_creaci FROM gloriadiaz.vs_exp_empresa WHERE CONVERT(fec_creaci, DATE)  = '$fecha_inicial'";
	// echo "<pre>";
	// print_r($queryR);
	// echo "</pre>";
	// die();
	...
	while ($row=mysqli_fetch_assoc($result)):
		...
		->setCellValue('I'.$c, $row['empresa'])
		->setCellValue('J'.$c, $row['USER'])
		->setCellValue('K'.$c, utf8_encode($row['organizador']))
		;
		$c++;
		$i++;
	endwhile;
	...
	header('Content-Disposition: attachment;filename="AsistenciaEmpresa.xlsx"');
	...
	echo "<pre>";
	echo "La variable fechha esta vacia.";
	echo "</pre>";
}